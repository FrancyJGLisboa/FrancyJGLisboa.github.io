...
```{r  fig7, echo=FALSE, out.width="100%", fig.cap='Snapshot of the aquastat_legacy dataset in SWS.'}
...
```{r  fig8, echo=FALSE, out.width="100%", fig.cap='Snapshot of the aquastat_baseline dataset as the benchmark (input) for the future updates of AQUASTAT data collected via questionnaire'}
...
```{block, type='rmdnote'}
For the moment, the **aquastat_legacy** and **aquastat_baseline** datasets are using the same SWS element reference list called **aquastatElement**, which does not align with SWS standards. Therefore, for the sake of standadization, the AQUASTAT element reference list in the SWS MUST be named **measuredElement** at some point in the future.
=======
...
>>>>>>> 7c4f3e555d7976d32d6dc32e799a80d001592b7b
```
...
```{block , type='rmdnote'}
In the **validation_rule** data table, some of the expressions are OK for the application of the primary variable rules. The rule grounds on the principle that some indicators have a primary variable that can be used as a substitute in case iof the indicator value is missing. This approach only applies to indicators obtained from addition and subtraction expressions. Therefore, the **PR** variable in the **aquastat_reference** data table is used to help the SWS modules to identify the elements in these expressions.
