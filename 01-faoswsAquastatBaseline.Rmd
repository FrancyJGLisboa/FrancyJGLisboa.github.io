...
The faoswsAquastatBaseline is meant to run and generates the updated benchmark for AQUASTAT activities in the SWS context. The module, therefore, seeks to fulfil AQUASTAT demands described below.
...
The technical unit has been re-disseminating FAOSTAT data as a way to provide supporting statistics in a single data dissemination platform. From a data processing perspective, an effective way to streamline the use of FAOSTAT data by AQUASTAT would be to take the data from data sets already residing in other SWS statistical domains. However, it has been proven to be problematic because the SWS data sets where the FAOSTAT data were supposed to be are actually empty. Currently, the AQUASTAT focal point responsible for the updates of the AQUASTAT working system has been proceeding as follow:
...
1. Get FAOSTAT data from FAOSTAT website;

  - [Land Use](http://www.fao.org/faostat/en/#data/RL);
  
  - [Population](http://www.fao.org/faostat/en/#data/OA);
  
  - [GDP deflator](http://www.fao.org/faostat/en/#data/PD) (**to calculate GDP deflator based on 2015 value**).
2. Map FAOSTAT to AQUASATAT code using the **aquastat_faostat_mapping** data table;

3. Read in the data set **aquastat_legacy** from SWS database (no indicators in the data set);

4. **Annex** the re coded FAOSTAT data to the AQUASTAT aquastat_enr data set to form a single data set used as **input** in the next steps of the faoswsAquastatBaseline Workflow.


```{block , type='rmdnote'}
Important to notice that while a new reference list for aquastat_baseline dataset is not available in the SWS, the reference list to used on this dataset will be the same as the one used on the aquasta_legacy data (aquastatElement). The AQUASTAT reference list for elements in datasets **from the aquastat_baseline** onwards will be called **measuredElement**. In this SWS new AQUASTAT reference list, FAOSTAT codes will be SWS compliant and no longe recoded in order to avoid code duplication.  
...
# Inputs
FAOSTAT [label = 'Dataset: \n Downloaded FAOSTAT data \n from FAOSTAT website', shape = folder, fillcolor = Beige]
SWSenr [label = 'Dataset: \n aquastat_legacy \n from Old AQUASTAT working system',  shape = folder, fillcolor = Beige]


FAOSTATSWS [label = ' Processing: \n FAOSTAT - AQUASTAT merge using:\n 1. faostat_mapping data table, \n 2. aquastat_legacy data, \n 3. FAOSTAT data', shape = square, fillcolor = LightBlue]


# Output
Dataset1 [label = 'Dataset: \n SWS-AQUASTAT dataset \n merged with FAOSTAT data', shape = folder, fillcolor = Beige]

# Flow
# edge definitions with the node IDs
{SWSenr, FAOSTAT} -> FAOSTATSWS -> Dataset1 
...
d = data.table::fread("tables/aquastat_meta_FL_sources.csv")
knitr::kable(x = d[PR == 1L, .(element_code, element_name, disseminated, calculated, source)], 
             caption = paste("Primary - rule applicable elements in the aquastat_reference data table"),
             booktabs = TRUE)
```

4. **Calculation** of indicators by applying the information in the **calculation_rule** data table (see Table \@ref(tab:tab2));

5. After calculations, replace missing values (NAs) by the value of the primary variable of the indicator when applicable following the rule:
...
The calculation step added new indicators but did not solve the issue of time-series completeness. The Imputation: 

1. Expands variables and indicators time-series **from the first observed value until 2018** as agreed by the technical unit;
...
3. Fills the generated missing values using the following guideline:
   ...
   - Applies **Last Observation Carried Forward** to missing values in time-series of Long-Term average variables (see Table \@ref(tab:tab3) for reference);
...
3. Re-applies the primary variable rule using the variables in the Table \@ref(tab:tab2);
...
The SWS adheres to the [Statistical Data and Metadata eXchange](https://sdmx.org/) (SDMX) guidelines to flag observations in data sets living in the system. The migration of the AQUASTAT dataset into the SWS does not perform any action towards the mapping between mapping between AQUASTAT flags and SWS flags. To solve it, the faoswsAquastatBaseline module maps AQUASTAT to valid SWS flags using the following conversion table:
...
Besides the basic conversion described above, values emerged from the faoswsAquastatBaseline module processing were flagged as follow:
...
3. The **M** becomes **I** because all M values were transformed into missing to avoid errors from linear interpolation.


### Flag mapping (Method)

With regard to the flag of methods in SWS, the conversion following criteria:
...
To run the faoswsAquastatBaseline after any updated of either aquastat_enr SWS data set or FAOSTAT - based variables, the user needs to:
