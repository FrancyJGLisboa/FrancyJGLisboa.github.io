...
With the availability of the outputs from the **faoswsAquastatBaseline** and **faoswsAquastatExternal** modules as well as the  ***aquastat_questionnaire**  dataset output by the questionnaire harvester by CIO - SWS, the faoswsAquastatUpdate module uses these three datasets as inputs. After feeding on the inputs, the module proceeds to harmonize the inputs by harmonizing their dimension names to **geographicAreaM49**, **measuredElement**, **timePointYears**, and **flagObservationStatus**.


### **replaces/updates the legacy data**

The **aquastat_questionnaire** and **aquastat_external** datasets are the brand new data used to update the legacy data represented by the **aquastat_baseline** dataset. Thus, the faoswsAquastatUpdate module:

1. **Finds** and **replaces** rows in the *aquastat_baseline* dataset by the corresponding rows of the *aquastat_aquestionnaire* dataset.

2. **Finds** and **replaces** rows in the *aquastat_baseline* dataset by the corresponding rows of the *aquastat_external* dataset.
...
3. Fills the generated missing values using the following guideline:
   ...
   - Applies **Last Observation Carried Forward** to missing values in time-series of Long-Term average variables (see Chapter \@ref(AquastatBaseline) for reference);
...
After the update of the aquastat_baseline by the aquastat_questionnaire and aquastat_external datasets, the the final step is the mapping between AQUASTAT and SWS flags. The faoswsAquastatUpdate module maps out AQUASTAT to valid SWS flags using the following conversion table:
<br>
...
3. Select **AQUASTAT domain** and **aquastat_baseline dataset**;

4. Select whatever country, measured element, and timePointYears;

```{block , type='rmdnote'}
The **faoswsAquastatUpdate** module updates the **aquastat_baseline** dataset (corrected legacy data) using the **aquastat_questionnarie** and **aquastat_export** datasets that were saved into the SWS database. It means that that the user's selections during this step are irrelevant for the module's output. 
