...
Apart from the questionnaire harvesting, the processes in green are those performed by AQUASTAT modules **already operating in the QA environment in the SWS**. The **faoswsAquastatUpdate module** is yet to be built after the final endorsement of the faoswsAquastaBaseline output by the technical unit. 
...
9. **aqua_calculation_rule**. A data table storing the calculation rules to apply to aggregation of the SDG 6.4.2 (Water Stress) at the regional level

10. **aqua_external_source**. A data table with the mapping between AQUASTAT and Data used by AQUASTAT from external agencies.
...
1. **aquastat_legacy**. Currently named as **aquastat_enr**, the **aquastat_legacy** is an SWS dataset with the legacy data from the 'Old' AQUASTAT working system. The CIO - SWS has migrated the aquastat_legacy dataset into the SWS in order to be used as input for the faoswsAquastatBaseline module (see \@ref(AquastatBaseline)) responsible for its correction. The **aquastat_legacy** dataset is the bare minimum of AQUASTAT, meaning that none analytical process has been applied to it, including the calculation of AQUASTAT indicators (aka derived variables). The corrected **aquastat_legacy** dataset is used, along with the questionnaire data and external data, as an input by the **faoswsAquastatUpdate** module.

2. **aquastat_baseline**. An SWS dataset output from the faoswsAquastatBaseline module which is going to correct the Long-Term average variables in the **aquastat_legacy dataset**. 

3. **aquastat_questionnaire**. An SWS dataset output from the questionnaire harvester produced by CIO. This dataset feeds, along with the aquastat_baseline and the aquastat_external datasets, the faoswsAquastatUpdate module.

4. **aquastat_external**. An SWS dataset output from the questionnaire harvester produced by CIO. This dataset feeds, along with the aquastat_baseline and the aquastat_quest datasets, the faoswsAquastatUpdate module.

5. **aquastat_update**. An SWS dataset output from the **faoswsAquastatUpdate** module representing the up-to-date AQUASTAT dataset for dissemination purposes.
